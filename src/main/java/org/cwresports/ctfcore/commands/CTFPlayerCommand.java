...
public class CTFPlayerCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean handleJoin(Player player, String[] args) {
        if (plugin.getCooldownManager().isOnCooldown(player, "join")) {
            long remaining = plugin.getCooldownManager().getRemainingCooldown(player, "join");
        ...
        plugin.getCooldownManager().setCooldown(player, "join", 3);
        return true;
    }

    private boolean handleLeave(Player player, String[] args) {
        if (plugin.getCooldownManager().isOnCooldown(player, "leave")) {
            long remaining = plugin.getCooldownManager().getRemainingCooldown(player, "leave");
            Map<String, String> placeholders = new HashMap<>();
            placeholders.put("seconds", String.valueOf(remaining));
            player.sendMessage(plugin.getConfigManager().getMessage("cooldown-command", placeholders));
            return true;
