...
public class ArenaManager {
    ...
    public ValidationResult validateAndEnableArena(String name) {
        ...
        // Try to save schematic, but don't fail arena validation if it fails
        try {
            saveArenaSchematic(arena);
        } catch (Exception e) {
            plugin.getLogger().warning("Failed to save schematic for arena " + name + " but arena was still enabled: " + e.getMessage());
        }
    ...
    public void saveArenaSchematic(Arena arena) {
        try {
            ...
            // Get WorldEdit world and region bounds
            ...
            if (min == null || max == null) {
                plugin.getLogger().warning("Could not get region bounds for arena " + arena.getName() + " - skipping schematic save");
                return;
            }
            ...
            // Get clipboard format explicitly for .schem files
            ClipboardFormat format = ClipboardFormats.findByAlias("schem");
            if (format == null) {
                // Fallback to sponge schematic format
                format = ClipboardFormats.findByAlias("sponge");
            }
            if (format == null) {
                plugin.getLogger().warning("No suitable clipboard format found for arena " + arena.getName() + " - skipping schematic save");
                return;
            }
            ...
            plugin.getLogger().info("Successfully saved schematic for arena " + arena.getName());
        } catch (IOException | WorldEditException e) {
            plugin.getLogger().severe("Failed to save schematic for arena " + arena.getName() + ": " + e.getMessage());
        } catch (Exception e) {
            plugin.getLogger().severe("Unexpected error saving schematic for arena " + arena.getName() + ": " + e.getMessage());
        }
    }
    ...
    public void restoreArena(Arena arena) {
        try {
            ...
            if (format == null) {
                plugin.getLogger().warning("No suitable clipboard format found for restoring arena " + arena.getName());
                return;
            }
            ...
            plugin.getLogger().info("Successfully restored schematic for arena " + arena.getName());
        } catch (IOException | WorldEditException e) {
            plugin.getLogger().severe("Failed to restore schematic for arena " + arena.getName() + ": " + e.getMessage());
        } catch (Exception e) {
            plugin.getLogger().severe("Unexpected error restoring schematic for arena " + arena.getName() + ": " + e.getMessage());
        }
    }
    ...
    public boolean startFlagSetup(Player player, String arenaName, Arena.TeamColor teamColor) {
        ...
        placeholders.put("team_color", teamColor.getColorCode());
        player.sendMessage(plugin.getConfigManager().getMessage("setup-flag-mode", placeholders));
    ...
    public boolean startCaptureSetup(Player player, String arenaName, Arena.TeamColor teamColor) {
        ...
        placeholders.put("team_name", teamColor.getName());
        placeholders.put("team_color", teamColor.getColorCode());
        player.sendMessage(plugin.getConfigManager().getMessage("setup-capture-mode", placeholders));
